...
const getUsersWithFriend = (users, friendName) => users.filter(({ friends }) => friends.includes(friendName)).map(user => user.name);

// const getUsersWithFriend = (users, friendName) => {

//   return users.reduce(function(acc, friend) {

//    if (friend.friends.some(friend => friend === friendName)) {
...
const getSortedUniqueSkills = users => {
  return users.reduce((acc, user) => [...acc, ...user.skills], []).filter((skill, index, arr) => arr.indexOf(skill) === index).sort();
};
